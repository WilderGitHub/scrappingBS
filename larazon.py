...
for pagina in range(desde, hasta+1, pasito):
    ...
    for n in range(len(links)):
        ...
        try:
            # print("tryando")
            indice = elarticulo.find_next(id).find_next(
                id).find_next(id).find_next(id)
      ...
      # ids.append(indice['id'])
        ...
        try:
            ...
            # print("ezept")
            divsparrafos = elarticulo.find_all(
                'div', class_="article-body", limit=1)
    ...
    troso = pd.DataFrame(
        list(zip(ids, fechas, titulares, detalles, textos)), columns=columnas)
